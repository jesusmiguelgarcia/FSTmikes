...
#------------marcadores con regexp o prefilter con regexp ----------------------------
   # @staticmethod
    ...
    @staticmethod
    def calc_markers(string, regexp, type_marker):
        
        string=string.replace(u"á",u"a").replace(u"é",u"e").replace(u"í",u"i").replace(u"ó",u"o").replace(u"ú",u"u")
        string = string.lower() 

        ma_premisas=["(ya que)","(puesto que)","(como)","(en tanto que)", "(dado que)","(viendo que)","(debido a)", "(de acuerdo con)", ("por cuanto")]
        ma_conclusiones=["(por lo tanto)","(por ende)", "(asi)","(de ahi que)","(en consecuencia)","(por consiguiente)","(se desprende de)", "(como resultado)", "(llegamos a la conclusion)"]
 

	final_tokens = []
	expresion=0
	for pc in ma_premisas:	
            expresion = re.findall(pc, string, re.I)
	    encontrados = len(expresion)
	    for i in range(encontrados):
	        final_tokens += ["marcador~premisa"]

	expresion=0
	for pc in ma_conclusiones:	
            expresion = re.findall(pc, string, re.I)
	    encontrados = len(expresion)
	    for i in range(encontrados):
	        final_tokens += ["marcador~conclusion"]
       
        #tokens = nltk.regexp_tokenize(string, regexp)
	#generar terminos - patrones - solo indicar: premisa_marker 
        #bigrams = nltk.bigrams(tokens)
        #final_tokens = []
        #for bigram in bigrams:
        #    final_tokens += ['~'.join(list(bigram))]
...
#from pynlpl.clients.freeling import FreeLingClient
    ...
    #    
    #     tagger = FreeLingClient(host,port) 
    #      
    #     words = []
    #     postags = []
    #     lemmas = []
    #     for fields in tagger.process(texto, debug):
    ...
    #         if pos is None: pos = "?"     
    #         tuplas.append([word,lemma,pos]) 
    #  
    #     return words, postags, lemmas, tuplas
    #===========================================================================
...
# con mode para definir con 0 - lemas 1 - tag 2 - tuplas
# regresa una lista con los elemntos
# se llama una vez por trabajo
...
#from pynlpl.clients.freeling import FreeLingClient
    ...
    def calc_POS_FREELING(string, pos, regexp):
        ...
        tokens = nltk.regexp_tokenize(string, regexp)
        final_tokens = tokens
        #mode 0 para obtener solo los memas en una lista
        ...
        if(pos == "lemas"):
            ...
            #tagger = FreeLingClient("localhost",50005) 
            #words, postags, lemmas, tuplas = POS_freeling(tokens)
            #print lemmas
        ...
        if(pos == "tags"):
            ...
            #print "mode1"
            #final_tokens = []
            #lema con tipo de elemento
            #for i in range(len(lemmas)):
                #unir = lemmas[i]+"~"+postags[i][0:3]
                #print unir
                #print lemmas
